/* Detail-panel basemap for the gift estimate card. Reference implementation.
   ...
   Usage:
     const png = await renderDetailMap({ zips: ['60637'], neighbors: chicagoZips, width: 400, height: 400 });
...
(function () {
   ...
   * opts.maxKm      widest selection that still gets a basemap, default 25
  ...
  function renderDetailMap(opts) {
    ...
    var W = opts.width || 400, H = opts.height || 400, timeout = opts.timeoutMs || 6000, maxKm = opts.maxKm || 25;
    ...
    return Promise.all([polygons(opts.zips), opts.neighbors && opts.neighbors.length ? polygons(opts.neighbors) : null]).then(function (r) {
      ...
      var bb = bbox(sel);
      if (widthKm(bb) > maxKm) return done(null); // too wide: the outline panel is the better picture
